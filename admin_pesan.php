    ...
    <div class="section">
        <div class="container">
            ...
            <table border="1" cellspacing="0" class="table">
                        ...
                        <?php
                            $no = 1;
                            $uploads = mysqli_query($connect, "SELECT uploads.*, kurir.nama FROM uploads 
                            LEFT JOIN kurir ON uploads.kurir_id = kurir.id 
                            ORDER BY uploads.id DESC");
                                ...
                                while ($row = mysqli_fetch_array($uploads)) {
                                    ...
                                    //dekripsi pesan
                                    $decrypted_message = decrypt_message($row['pesan'], $xor_key, $rails);
                                    $pesan = $decrypted_message;

                                    //Membuat Directory untuk secret image
                                    $secret_dir = 'uploads/secret_images/';
                                    if (!is_dir($secret_dir)) {
                                        mkdir($secret_dir, 0777, true);
                                    }
                                    $secretPath = $secret_dir . $row['id'] . '_secret.png';

                                    //Reveal Secret Image
                                    extract_secret_image($row['stego_image_path'], $secretPath);

                                    //membuat direktori untuk decrypted file
                                    $decryptedDir = 'uploads/decrypted_files/';
                                    if (!is_dir($decryptedDir)) {
                                        mkdir($decryptedDir, 0777, true);
                                    }
                                    $decryptedFileName = time() . '_decrypted_' . basename($row['file_path']);
                                    $decryptedFilePath = $decryptedDir . $decryptedFileName;

                                    //decrypt encyprted file
                                    aes_decrypt_file($row['file_path'], $decryptedFilePath, $aes_key);
                            ?>
                            <tr>
                                <td><?php echo $no++; ?></td>
                                <td><?php echo $row['nama']; ?></td>
                                <td><?php echo $pesan; ?></td>
                                <td><img src="<?php echo $row['stego_image_path']; ?>" width="200px"></td>
                                <td><img src="<?php echo $secretPath ?>" width="200px"></td>
                                <td>
                                    <?php  echo '<a href="' . $decryptedFilePath . '" target="_blank">Show File</a>';?>
                                </td>
                            </tr>
                            <?php }}else{ ?>
                                <tr>
